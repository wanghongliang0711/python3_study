...
# 复制文件
def copy_file(q, one_file, new_path):
    f_read = None
    f_write = None
    try:
...
def main():
    ...
    for one_file in all_files:
        po.apply_async(copy_file, args=(q, one_file, new_path))
    ...
    while True:
        ...
        copy_ok_num += 1
        print("\r拷贝的进度为: %.2f %%" % (copy_ok_num*100/all_files_num), end="")
        if copy_ok_num >= all_files_num:
            break
